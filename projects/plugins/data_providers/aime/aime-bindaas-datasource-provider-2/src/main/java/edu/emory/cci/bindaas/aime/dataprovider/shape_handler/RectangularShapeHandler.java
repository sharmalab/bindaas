...
/**
 * A Rectangular Shap is represented by PolyLine object with 4 lines
 * @author nadir
 *
 */

public class RectangularShapeHandler implements IShapeHandler{

	private String xpathExpression = "/ns1:ImageAnnotation/ns1:geometricShapeCollection/ns1:GeometricShape[@xsi:type='Polyline']";
	...
	public MarkupInfo extractMarkupInformation(Document doc, XPath xpath)
		...
		if (nodes.getLength() > 0) {
			...
			for(int i = 0  ; i < nodes.getLength() ; i ++ )
			{
				...
				if(spatialCoordinateNodeList.getLength() == 4)
				{
				...
				{
					log.warn("Illegal number of Points encountered in Rectangular Shape. Found [" + spatialCoordinateNodeList.getLength() + "] expected [4]");
					continue; // move on to the next one as this one is malformed
				}
				
				if(coordinateGroup.getCoordinates().size() == 4)
				{

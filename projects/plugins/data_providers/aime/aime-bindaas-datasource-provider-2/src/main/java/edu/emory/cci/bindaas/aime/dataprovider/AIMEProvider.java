...
public class AIMEProvider implements IProvider{
	...
	private  String testAIMETableQuery;
	...
	public String getTestAIMETableQuery() {
		return testAIMETableQuery;
	}

	public void setTestAIMETableQuery(String testAIMETableQuery) {
		this.testAIMETableQuery = testAIMETableQuery;
	}
	...
	private String getProviderTemplate(String tableName) throws IOException
	{
		InputStream is = Activator.getContext().getBundle().getEntry("META-INF/datasource-template.json").openStream();
		String template = IOUtils.toString(is); 
		return template.replaceAll("%s", tableName);
	}
	...
	public Profile validateAndInitializeProfile(Profile profile)
		...
		try {
			...
			if(configuration.isAutoInitialize() == true)
			{
				...
				String providerTemplate = getProviderTemplate(configuration.getTableName());
				
				Profile profileTemplate = GSONUtil.getGSONInstance().fromJson(providerTemplate, Profile.class);
...
//				
//				JsonObject properties  = GSONUtil.getGSONInstance().toJsonTree(seProps).getAsJsonObject(); 
//				submitEndpoint.setProperties(properties);
//				profile.getSubmitEndpoints().put(submitEndpoint.getName(), submitEndpoint);
			}
			
			profile.setDataSource(GSONUtil.getGSONInstance().toJsonTree(configuration).getAsJsonObject());
	...
	private void autoInitializeProvider(DataSourceConfiguration configuration) throws Exception {
		...
		try {
			...
			String createTableQuery = String.format(createAIMETableQuery,tableName);
			String dropTableQuery = String.format(dropAIMETableQuery, tableName);
			Statement statement = connection.createStatement();
			boolean isTableExist = isTableExist(tableName, statement);
			
			if(isTableExist && configuration.isDropTable())
			{
				// table exist and drop table
				log.debug("Dropping and creating table [" + tableName + "]");
				statement.execute(dropTableQuery);
				statement.executeUpdate(createTableQuery);
			}
			else if(isTableExist && configuration.isDropTable() == false)
			{
				// table exist but dont drop the table
				return;
			}
			else
			{
				// table does not exist so create table
				log.debug("Creating new table [" + tableName + "]");
				statement.executeUpdate(createTableQuery);
			}
	...
	public boolean isTableExist(String tableName , Statement st)
	{
		try{
			String testQuery = String.format(this.testAIMETableQuery, tableName);
